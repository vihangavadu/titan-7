...
class FingerprintInjector:
    ...
    def write_user_js(self, profile_path: Path) -> Path:
        ...
        user_js_lines = [
            ...
            '',
            '// ── Hardware Concurrency Lock (PRIORITY 1 PATCH) ──────────────',
            '// Critical: Sync kernel-spoofed CPU cores with browser navigator',
            f'user_pref("dom.hardwareConcurrency", {self.config.hardware_profile.get("cpu_count", 8) if self.config.hardware_profile else 8});',
            '',
            '// ── Device Memory Lock ──────────────────────────────────────',
            '// Critical: Sync kernel-spoofed RAM with browser navigator (buckets: 0.25,0.5,1,2,4,8)',
            f'user_pref("device.memory", {min(8, self.config.hardware_profile.get("ram_gb", 16)) if self.config.hardware_profile else 8});',
            '',
