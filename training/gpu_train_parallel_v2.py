...
Optimized for A100 SXM4 40GB VRAM.
"""
...
# A100 40GB optimized: batch=2, reduced max_seq to prevent OOM
LORA_CFG = {
    "strategist": {"r": 16, "alpha": 32, "dropout": 0.05, "lr": 1.2e-4, "epochs": 4, "max_seq": 1024, "batch": 2, "grad_accum": 8},
    "fast":       {"r": 12, "alpha": 24, "dropout": 0.05, "lr": 1.8e-4, "epochs": 4, "max_seq": 768,  "batch": 2, "grad_accum": 4},
}
...
def train_model(group):
    ...
    print(" LoRA: r=%d alpha=%d batch=%d grad_accum=%d max_seq=%d" % (
        cfg["r"], cfg["alpha"], cfg["batch"], cfg["grad_accum"], cfg["max_seq"]))
    print("=" * 60)
    ...
    if len(examples) == 0:
        print("  ERROR: No training data! Skipping %s." % group)
        return False
    ...
    print("\n[3/5] Loading model to CPU then moving to CUDA...")
    import gc
    ...
    print("  Loaded: %.1fB params, Free VRAM: %.1f GB" % (param_b, free_mem / 1e9))
...
if __name__ == "__main__":
    print("=" * 60)
    print(" TITAN V9.0 — Parallel GPU Training (A100 SXM4 40GB)")
    print(" Training: strategist + fast (analyst on 3090 Ti)")
    print("=" * 60)
    ...
    print("[DONE] Parallel training complete! Models: %s" % ", ".join(success))
