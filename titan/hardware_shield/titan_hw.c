...
static int __init titan_hw_init(void)
{
    ...
    */
    ...
    dmi_kobj = kobject_create_and_add("titan_dmi", kernel_kobj);
    if (dmi_kobj) {
        int r1 = sysfs_create_file(dmi_kobj, &dmi_sys_vendor.attr);
        int r2 = sysfs_create_file(dmi_kobj, &dmi_prod_name.attr);
        int r3 = sysfs_create_file(dmi_kobj, &dmi_prod_uuid.attr);
