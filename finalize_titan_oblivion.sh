...
# Safety check: if directories don't exist, skip silently
if [ ! -d "$ISO_CONFIG_DIR/includes.chroot" ]; then
    echo "[!] Warning: $ISO_CONFIG_DIR/includes.chroot not found"
    echo "    Skipping detailed finalization (will be created by build process)"
    exit 0
fi
...
find "$SOURCE_DIR" -type f -name "*.py" -print0 2>/dev/null | xargs -0 sed -i '/GitHub Copilot/d' 2>/dev/null || true
find "$SOURCE_DIR" -type f -name "*.py" -print0 2>/dev/null | xargs -0 sed -i '/Generated by AI/d' 2>/dev/null || true
find "$SOURCE_DIR" -type f -name "*.py" -print0 2>/dev/null | xargs -0 sed -i '/# TODO:/d' 2>/dev/null || true
find "$SOURCE_DIR" -type f -name "*.py" -print0 2>/dev/null | xargs -0 sed -i '/# FIXME:/d' 2>/dev/null || true

# 1.2 Strip Metadata from Python Files & Normalize Shebangs
# Ensures uniform environment execution paths.
find "$SOURCE_DIR" -type f -name "*.py" 2>/dev/null -exec sed -i '1s|^#!.*|#!/usr/bin/env python3|' {} + || true
...
if [ -d "$PROFGEN_DIR" ]; then
    find "$PROFGEN_DIR" -type f -name "*.py" -print0 2>/dev/null | xargs -0 sed -i 's|/Users/|C:\\\\Users\\\\|g' || true
...
mkdir -p "$ISO_CONFIG_DIR/includes.chroot/etc/sysctl.d" 2>/dev/null || true
cat << EOF > "$ISO_CONFIG_DIR/includes.chroot/etc/sysctl.d/99-titan-stealth.conf" 2>/dev/null || true
...
mkdir -p "$ISO_CONFIG_DIR/includes.chroot/etc/security/limits.d" 2>/dev/null || true
echo "* hard core 0" > "$ISO_CONFIG_DIR/includes.chroot/etc/security/limits.d/disable-cores.conf" 2>/dev/null || true
